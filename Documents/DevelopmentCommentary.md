...
**Auraline** is an interactive audio-visual synthesis application built in **Unity** [(Unity Technologies, 2026)](https://unity.com) that bridges the gap between digital sound design and physical gesture. The project explores the intersection of psychology, visual art, and audio perception, focusing on how human beings process and interact with multisensory stimuli. While stylistically and technically inspired by historical systems like the **Xenakis UPIC** [(Wikipedia, 2026b)](https://en.wikipedia.org/wiki/UPIC) and the **Oramics Machine** [(Wikipedia, 2026a)](https://en.wikipedia.org/wiki/Oramics), the application eschews traditional, button-heavy interfaces in favor of a clean, gesture-based "drawing-to-sound" experience driven by the **FMOD** [(Firelight Technologies, 2026)](https://www.fmod.com) audio engine. A core feature is the implementation of a theatrical "Power-Up" sequence and reactive material emissions that provide immediate visual feedback for real-time audio modulation.

For instance, the application utilizes a mix of 2D and 3D interactive drum pad and a **"Ghost Pen"** tutorial system to guide users through the synthesis process. The pen leaves a dynamic trail teaching the user the mechanics of the interface before they begin their own performance. Importantly, the system utilizes sequential visual "breadcrumbs," where elements like the **Reset** and **Next Track** buttons only fade into existence as the user requires them, prioritizing an immersive, minimal interface over traditional HUD-heavy layouts.
...
- Creating a highly optimized, lightweight rendering pipeline that guarantees a rock-solid, high-framerate experience on portable hardware like a MacBook Air [(Apple Inc., 2026a)](https://www.apple.com/macbook-air/), supported by modular C# code that allows developer-friendly expansions for future audio tracks and pad layouts.
...
> "Legacy, community-made integration tutorials are hopelessly broken due to massive core architecture rewrites, and authors rarely update their source material to reflect updated node or script behavior." [(FMOD Q&A Forum, 2024)](https://qa.fmod.com/t/lots-of-warnings-in-unity-6-2/23701)
...
I kicked off development by bridging the gap between tactile performance and fluid, visual sound synthesis. The goal was to establish a clear visual blueprint before diving into Unity, resulting in the Auraline AP-10 concept mock-up. I have asked **Gemini** [(Google, 2026)](https://gemini.google.com/app) to generate a 2D model of a drum pad with a screen used for drawing and modifying sounds. This was the result:
...
Switching to **ProBuilder** [(Unity Technologies, 2024g)](https://docs.unity3d.com/Packages/com.unity.probuilder@6.0/manual/index.html) directly within Unity streamlined the pipeline:
...
According to the official **FMOD Studio Concepts Manual** [(Firelight Technologies, 2024d)](https://www.fmod.com/docs/2.03/studio/fmod-studio-concepts.html), an **Event** is defined as:
...
* **Low-Overhead DSP Pipeline:** Passing these normalized values directly into FMOD’s `setParameterByName` [(Firelight Technologies, 2024a)](https://www.fmod.com/docs/2.03/unity/api-runtimemanager.html) bypasses heavy intermediate calculations. This delivers highly efficient, sample-accurate runtime modulation over pitch and reverb directly within the native audio mixer.
...
According to the official FMOD Studio Instruments Manual: Nested Events [(Firelight Technologies, 2024e)](https://www.fmod.com/docs/2.03/studio/authoring-events.html#nested-events), nested events provide an elegant structural solution:
...
* **Minimized Memory Overhead:** Instead of continuously destroying and instantiating new `EventInstance` blocks at runtime—which risks memory fragmentation—Unity maintains a single parent instance. Track switching is handled efficiently via a global parameter (`TrackSelector`), safeguarding framerate stability on portable hardware like the MacBook Air [(Apple Inc., 2026a)](https://www.apple.com/macbook-air/).
...
* **Parameter Scope Mismatch:** The C# script invoked `RuntimeManager.StudioSystem.setParameterByName()` [(Firelight Technologies, 2024a)](https://www.fmod.com/docs/2.03/unity/api-runtimemanager.html), which broadcasts across FMOD's global parameter pipeline. However, the `TrackSelector` parameter inside FMOD Studio was configured as a **Local (Instance-based)** variable. Because of this scope mismatch, Unity's global calls were ignored by the local event instance.
...
* **Streaming Discovery Frameworks:** I utilized Spotify [(Spotify, 2026)](https://www.spotify.com) and YouTube [(YouTube, 2026)](https://www.youtube.com) as my primary hunting grounds, digging deep into archival world-music playlists, multi-cultural radio broadcasts, and isolated performance stems.
* **Pro-Grade Sample Acquisition:** For high-fidelity source material that could be cleanly manipulated, I leveraged Tracklib [(Tracklib, 2026)](https://www.tracklib.com/) to find, isolate, and legally source specialized master loops with precise key and tempo data.
...
This track is build around the sample **"Caravan"** written by [Thelonious Monk](https://en.wikipedia.org/wiki/Thelonious_Monk). Another sample that has challenged me to experiment with a different approach and genre, which it was exactly what I needed to slowly build a chameleonic style when it comes to musical arrangements. The integrity of the sample has been kept, the only thing that I have done to it was to cut it short for **Auraline**.
...
The project underwent **User Testing** (specifically, **observational usability testing** [(Nielsen, 1993)](https://en.wikipedia.org/wiki/Usability_testing)).
...
* **Distribution Strategy:** Balancing the split opinions on pricing and credit, the decision was made to open-source the project via a **GitHub Repository** [(GitHub, 2026)](https://github.com/) and launch the tool publicly on **Itch.io** [(Itch.io, 2026)](https://itch.io/), satisfying both the desire for accessibility/creditation and the framework for future indie distribution.
...
* **Apple Inc.** (2026a) *MacBook Air*. At: [https://www.apple.com/macbook-air/](https://www.apple.com/macbook-air/) (Accessed 19/05/2026).
* **Apple Inc.** (2026b) *Logic Pro*. At: [https://www.apple.com/logic-pro/](https://www.apple.com/logic-pro/) (Accessed 19/05/2026).
...
* **GitHub** (2026) *GitHub*. At: [https://github.com/](https://github.com/) (Accessed 19/05/2026).
* **Google** (2026) *Gemini Web Interface*. At: [https://gemini.google.com/app](https://gemini.google.com/app) (Accessed 19/05/2026).
* **Itch.io** (2026) *Itch.io*. At: [https://itch.io/](https://itch.io/) (Accessed 19/05/2026).
...
* **University for the Creative Arts (UCA)** (2026) *SmartHub*. At: [https://www.uca.ac.uk/](https://www.uca.ac.uk/) (Accessed 19/05/2026).
...
* **Wikipedia** (2026c) *Thelonious Monk Discography and History*. At: [https://en.wikipedia.org/wiki/Thelonious_Monk](https://en.wikipedia.org/wiki/Thelonious_Monk) (Accessed 18/05/2026).
...
Scripts were generated by Gemini, modified with Antigravity as development went on:

* AuralinePlayer.cs
* AuralineResetButton.cs
* Auraline_ScreenShake.cs
* Auraline_Knob.cs
* Drawing_Hint.cs
* Interactive_Pads.cs
* Light_PlayButton.cs
...
All of the scripts above can be found written entirely in [Scripts](./Scripts)

---

Asset generated by Gemini:
